...
def load_data():
    ...
    df=pd.read_csv(url)
    return df[df["REGION"] == "DKL"]

def run():
    set_theme()
    st.markdown("<h1 style='text-align: center;'>AD DAKHLIYAH </h1>", unsafe_allow_html=True)
    ...
    with left_col:
        ...
        st.markdown(
            ...
            text-align: center; margin-bottom: 41px; margin-left: 13px; padding-top: 15px; padding-left: 25px">
                <h5 style="margin-bottom:0px; font-family: 'Arial', serif; white-space: nowrap; color:white; font-size: 14px"> Galfar: 16</h5>
            </div>
            """,
            unsafe_allow_html=True
        )
