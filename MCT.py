...
def load_data():
    ...
    df=pd.read_csv(url)
    return df[df["REGION"] == "MCT"]

def run():
    set_theme()
    st.markdown("<h1 style='text-align: center;'> MUSCAT </h1>", unsafe_allow_html=True)
    st.markdown("<hr style='border: 1px solid #ccc;'>", unsafe_allow_html=True)
    df = load_data()



    wilayat_labels = ["SEEB", "BAWSHAR", "GMAQ"]

    col1, col2, col3 = st.columns([ 1, 3, 1])


    col_check1, col_check2, col_check3 = col2.columns(3)
    wilayat_cols = [col_check1, col_check2, col_check3]    
    
    selected_wilayats = []
    for i, wilayat in enumerate(wilayat_labels):
        if wilayat_cols[i%3].checkbox(wilayat, value=True, key=f"wilayat_{wilayat}"):
            selected_wilayats.append(wilayat)
    
    # Fallback if none selected
    if not selected_wilayats:
        df = df[df["WILAYAT"].isin([])]  # Empty dataframe to avoid showing anything
    else:
        df = df[df["WILAYAT"].isin(selected_wilayats)]
    ...
    with left_col:
        ...
        st.markdown(
            ...
            display: flex; align-items: center; justify-content: center; 
            text-align: center; margin-bottom: 6px; margin-left: 13px; padding-top: 15px; padding-left: 25px">
                <h5 style="margin-bottom:0px; font-family: 'Arial',serif; font-weight :1000; white-space: nowrap; color:#2D6A4F; font-size: 10px; ">PROJECT MANAGER</h5>
